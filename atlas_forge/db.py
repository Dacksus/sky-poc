...
from sqlalchemy import create_engine, DDL, Engine, Select, select, update
...
def validate_triggers():
    ...
    with engine.connect() as conn:
        result = conn.execute(sa.text("SELECT * FROM validate_version_consistency()")).fetchall()
        inconsistencies = [row for row in result if not (row.metadata_consistent and row.content_consistent)]
...
def recreate_triggers():
    ...
    from app.database_triggers import drop_triggers, create_triggers
...
def db_get_latest_metadata_for_element(elment_id: str) -> DocumentElementMetadata:
    stmt = select(DocumentElementMetadata).order_by(DocumentElementMetadata.version.desc())
    with Session() as session:
        return session.scalars(stmt).first()
...
def db_get_latest_content_for_element(element_id: str) -> DocumentElementContent:
    stmt = select(
        DocumentElementContent
            ...
            DocumentElementContent.document_element_id == element_id
        ).order_by(
            DocumentElementContent.version.desc()
        )
    with Session() as session:
        return session.scalars(stmt).first()
...
def db_get_latest_elements_by_document(document_id: uuid.UUID) -> list[(DocumentElementMetadata, DocumentElementContent)]:
    stmt = select(
        DocumentElement, DocumentElementMetadata
        ).join(
            DocumentElementMetadata, DocumentElement.id == DocumentElementMetadata.document_element_id
        ).where(
...
def db_get_previous_snapshot(snapshot_id: uuid.UUID) -> Snapshot:
    document_stmt = select(Snapshot.document_id, Snapshot.triggered_at).where(Snapshot.id == snapshot_id)
    subq = document_stmt.subquery()
    stmt = select(Snapshot).where(
        Snapshot.document_id == subq.c.document_id
        ).filter(
            Snapshot.triggered_at < subq.c.triggered_at
        ).order_by(
            Snapshot.triggered_at.desc()
        )
    with Session() as session:
        return session.scalars(stmt).first()